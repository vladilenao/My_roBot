...
**Reason**: Избыточны при явной передаче параметров через dataclass. `MacdIndicator(fast=12, slow=26, signal=9)` столь же читаем, как `.set_fast(12).set_slow(26).set_signal(9).build()`.
**Migration**: Создавайте индикаторы напрямую: `MacdIndicator(fast=12, slow=26, signal=9)`, `RsiIndicator(period=14)`, `StochasticIndicator(k=14, d=3, smooth_k=3)`.
