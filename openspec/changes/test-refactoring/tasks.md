...
- [ ] 4.1 Полный прогон `pytest` — 99 passed; выборочные прогоны `pytest tests/unit` и `pytest tests/snapshot` дают согласованные подмножества
- [ ] 4.2 Grep по репозиторию на устаревшие упоминания `tests/data` и `tests/strategies` вне архива openspec — правки или подтверждение отсутствия
- [ ] 4.3 Проверить pre-commit хук коммитом: хук запускает pytest из корня, оба подпространства обнаруживаются
