...
class BrokerExecutionAdapter:
    ...
    Дедупликация входа: максимум один входящий ордер на пару (инструмент, ТФ)
    за сигнальный бар. Повторный вход с тем же `bar_time` ордер не создаёт, а в
    журнале фиксируется причина `duplicate` (карта помнит только последний бар
    на пару и не растёт). Выходы и доборы не дедуплицируются.
    """
    ...
    def execute(
        ...
        *,
        ...
        if getattr(decision, "action", None) == "entry":
            duplicate = self._guard_entry_once(
                ticker, signal.timeframe, getattr(decision, "bar_time", None), signal, now
            )
    ...
    def _guard_entry_once(
        self, ticker: str, timeframe: str, bar_time, signal: Signal, now: datetime
    ) -> object | None:
        """Ограничивает вход: один ордер на (инструмент, ТФ) за бар; дубль — причина `duplicate`."""
        if bar_time is None:
            return None
        pair = (ticker, timeframe or "")
        if self._last_entry_bar.get(pair) == bar_time:
            log.info(
                "Вход %s (%s): сигнал за бар %s уже исполнен — дубликат, ордер не создаётся",
                ticker, timeframe, bar_time,
            )
            if isinstance(self.broker, JournalBroker):
                return self.broker.record_rejection(signal, "duplicate", now)
            return None
        self._last_entry_bar[pair] = bar_time
        return None
