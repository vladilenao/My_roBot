...
class JournalBroker(BrokerPort):
    ...
    def track_bar(
        ...
        best_long = max((lows[p.ticker] for p in self.manager.positions.values() if p.side == "BUY"), default=None)
        best_short = min((highs[p.ticker] for p in self.manager.positions.values() if p.side == "SELL"), default=None)
